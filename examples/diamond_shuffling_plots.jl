...
# ╔═╡ 62b3cfad-c959-475b-a98d-7b6362218ba2
begin
	Random.seed!(1)
	D = diamond(N)
end
...
let
	...
	hidedecorations!(ax)
	hidespines!(ax)
	img = image!(ax, [!(AztecDiamonds.in_diamond(N, I[1], I[2]) && isodd(I[1] + I[2] - N)) for I in CartesianIndices(AztecDiamonds.inds(N))]; interpolate = false, alpha = 0.5)
	translate!(img, -N, -N)
	plot!(ax, D; domino_stroke = 5, domino_padding = 0, colormap = RGBA.([colorant"red", colorant"green", colorant"yellow", colorant"blue"], 0.5))
	fig
end
...
let
	...
	plot!(ax, D; domino_stroke = 3, domino_padding = 0.1, colormap = RGBA.([colorant"red", colorant"green", colorant"yellow", colorant"blue"], 0.5), show_arrows = true)
	fig
end
...
let
	...
	plot!(ax, D1; domino_stroke = 3, domino_padding = 0.1, colormap = RGBA.([colorant"red", colorant"green", colorant"yellow", colorant"blue"], 0.5), show_arrows = true)
	fig
end
...
let N = N + 1
	...
	plot!(ax, D2; domino_stroke = 3, domino_padding = 0.1, colormap = RGBA.([colorant"red", colorant"green", colorant"yellow", colorant"blue"], 0.5), show_arrows = true)
	fig
end
...
[sources]
AztecDiamonds = {path = "/home/simeon/.julia/dev/AztecDiamonds"}
...
path = "/home/simeon/.julia/dev/AztecDiamonds"
